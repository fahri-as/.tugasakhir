<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-gray-50 to-blue-50 p-6 rounded-lg shadow-lg mb-6 border border-blue-100">
                ...
                <form method="GET" action="{{ route('magang.smartDashboard') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-lg mb-6 border border-gray-200">
                <div class="bg-gradient-to-r from-green-500 to-emerald-600 p-6">
                    <h3 class="text-xl font-bold text-white flex items-center">
                        <i class="fas fa-trophy text-yellow-300 mr-2"></i> Overall SMART Rankings
                    </h3>
                    <p class="text-green-100 text-sm mt-1">Performance rankings based on SMART calculations</p>
                </div>
                <div class="p-6">
                    @if($interns->isEmpty())
                    <div class="text-center py-12 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg">
                        <i class="fas fa-inbox text-gray-300 text-5xl mb-4"></i>
                        <p class="text-gray-500 text-lg">No active interns found for this position and period.</p>
                        <p class="text-gray-400 text-sm mt-2">Check your filter settings or add new interns</p>
                    </div>
                    @else
                    <!-- Top 3 Cards -->
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        @foreach($interns->take(3) as $index => $intern)
                    ...
                    <div class="overflow-x-auto bg-gray-50 rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center justify-center h-8 w-8 rounded-full {{ $intern->rank == 1 ? 'bg-yellow-100 text-yellow-800' : ($intern->rank == 2 ? 'bg-gray-100 text-gray-800' : ($intern->rank == 3 ? 'bg-orange-100 text-orange-800' : 'bg-gray-50 text-gray-600')) }} text-sm font-bold">
                                            {{ $intern->rank ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-8 h-8 bg-gradient-to-r from-indigo-400 to-purple-400 rounded-full flex items-center justify-center text-white text-xs font-bold mr-3">
                                                {{ substr($intern->pelamar->nama, 0, 1) }}
                                            </div>
                                            <span class="text-sm font-medium text-gray-900">{{ $intern->pelamar->nama }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('magang.show', $intern) }}" class="inline-flex items-center px-3 py-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-medium rounded-full hover:from-indigo-600 hover:to-purple-700 transform transition duration-150 hover:scale-105">
            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-lg mb-6 border border-gray-200">
                <div class="bg-gradient-to-r from-blue-500 to-indigo-600 p-6">
                    <h3 class="text-xl font-bold text-white flex items-center">
                        <i class="fas fa-chart-line text-white mr-2"></i> Weekly Performance Analysis
                    </h3>
                    <p class="text-blue-100 text-sm mt-1">Track performance trends across weeks</p>
                </div>
                <div class="p-6">
                    <!-- Week Tabs -->
                    <div class="border-b border-gray-200 mb-6">
                        <div class="flex overflow-x-auto scrollbar-hide pb-1">
                            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                                ...
                                <button type="button" class="week-tab {{ $week == 1 ? 'border-indigo-500 text-indigo-600 bg-indigo-50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-6 border-b-2 font-medium text-sm rounded-t-lg transition-all duration-200" data-week="{{ $week }}">
                        ...
                        <div class="overflow-x-auto bg-gray-50 rounded-lg">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center justify-center h-8 w-8 rounded-full {{ $ranking['rank'] == 1 ? 'bg-yellow-100 text-yellow-800' : ($ranking['rank'] == 2 ? 'bg-gray-100 text-gray-800' : ($ranking['rank'] == 3 ? 'bg-orange-100 text-orange-800' : 'bg-gray-50 text-gray-600')) }} text-sm font-bold">
                                                {{ $ranking['rank'] }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
    ...
    <script>
        ...
        function animatePanel(week) {
            ...
            const cards = panel.querySelectorAll('tr');
