    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @php
                ...
                $query = App\Models\Interview::query();
                ...
                // Apply status filters same as the main query, but don't limit stats by status when showing statuses
                ...
                $pendingInterviews = (clone $query)->where('status_seleksi', 'Pending')->count();
                $failedInterviews = (clone $query)->where('status_seleksi', 'Tidak Lulus')->count();
                $passedInterviews = (clone $query)->where('status_seleksi', 'Tes Kemampuan')->count();
            @endphp
                        ...
                        <form action="{{ route('interview.index') }}" method="GET" id="filter-form">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="transform transition duration-200 hover:-translate-y-1 md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Interview Status</label>
                                    <div class="grid grid-cols-3 gap-3 p-3 bg-white rounded-md border border-gray-200">
                                        ...
                                        <div class="flex items-center">
                                            <input id="status_test" name="statuses[]" type="checkbox" value="Tes Kemampuan"
                                                class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded"
                                                {{ in_array('Tes Kemampuan', (array)$selectedStatuses) ? 'checked' : '' }}>
                                            <label for="status_test" class="ml-2 text-sm font-medium text-gray-700 flex items-center">
                                ...
                                @forelse($interviews as $interview)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @if($interview->pelamar->job->job_id == 'JOB001') bg-orange-100 text-orange-800
                                            @elseif($interview->pelamar->job->job_id == 'JOB004') bg-pink-100 text-pink-800
                                            @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'chef') !== false) bg-green-100 text-green-800
                                            @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'waiter') !== false) bg-blue-100 text-blue-800
                                            @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'barista') !== false) bg-amber-100 text-amber-800
                                            @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'manager') !== false) bg-gray-100 text-gray-800
                                            @else bg-indigo-100 text-indigo-800 @endif">
                                                <i class="fas
                                                @if($interview->pelamar->job->job_id == 'JOB001') fa-utensils
                                                @elseif($interview->pelamar->job->job_id == 'JOB004') fa-birthday-cake
                                                @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'chef') !== false) fa-hat-chef
                                                @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'waiter') !== false) fa-concierge-bell
                                                @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'barista') !== false) fa-coffee
                                                @elseif(strpos(strtolower($interview->pelamar->job->nama_job), 'manager') !== false) fa-user-tie
