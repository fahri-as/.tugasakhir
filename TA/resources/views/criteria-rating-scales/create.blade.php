    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Create Rating Scale') }}
            </h2>
            <a href="{{ route('criteria.show', $criteria->criteria_id) }}" class="inline-block px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 active:bg-gray-300 focus:outline-none focus:bg-gray-300 transition ease-in-out duration-150">
                Back to Criteria
            </a>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('criteria-rating-scales.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="rating_level" name="rating_level" type="number" class="mt-1 block w-full"
                                :value="old('rating_level')" min="1" max="5" required />
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Create Rating Scale') }}</x-primary-button>
