<x-crud-layout title="Skill Test Management" createRoute="{{ route('tes-kemampuan.create') }}" createIcon="fa-vial" createText="Schedule New Test">
    <!-- Stats Overview -->
    @php
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 rounded-lg shadow-sm p-4 flex items-center transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-2xl font-semibold text-gray-800">{{ $totalTests }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-100 rounded-lg shadow-sm p-4 flex items-center transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-2xl font-semibold text-gray-800">{{ $pendingTests }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-red-50 to-rose-50 border border-red-100 rounded-lg shadow-sm p-4 flex items-center transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-2xl font-semibold text-gray-800">{{ $failedTests }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-lg shadow-sm p-4 flex items-center transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
            ...
            <div>
                ...
                <p class="text-2xl font-semibold text-gray-800">{{ $passedTests }}</p>
            </div>
        </div>

        <!-- Internship -->
        <div class="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 rounded-lg shadow-sm p-4 flex items-center transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
            <div class="rounded-full h-12 w-12 flex items-center justify-center bg-blue-100 text-blue-600 mr-4">
                <i class="fas fa-graduation-cap text-xl"></i>
            </div>
            <div>
                <p class="text-gray-500 text-sm">Internship</p>
                <p class="text-2xl font-semibold text-gray-800">{{ $internshipTests }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 bg-gradient-to-r from-gray-50 to-blue-50 p-5 rounded-lg shadow-sm border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-800 mb-4 flex items-center">
            <i class="fas fa-filter text-indigo-600 mr-2"></i> Filter Tests
        </h3>
        <form action="{{ route('tes-kemampuan.index') }}" method="GET" id="filter-form">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="transform transition duration-200 hover:-translate-y-1">
                    ...
                    <div class="relative">
                        ...
                        <select id="periode_filter" name="periode_id" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm">
                            @php
            ...
            <div class="flex items-center mt-4">
                <button type="submit" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:from-purple-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm hover:shadow">
                ...
                <tr>
                    <th scope="col" class="px-6 py-3 bg-gradient-to-r from-gray-50 to-gray-100 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="{{ route('tes-kemampuan.index', array_merge(request()->except(['sort_by', 'sort_dir']), ['sort_by' => 'tes_id', 'sort_dir' => $sortBy == 'tes_id' && $sortDir == 'asc' ? 'desc' : 'asc'])) }}" class="group inline-flex items-center">
                            ...
                            @if($sortBy == 'tes_id')
                                @if($sortDir == 'asc')
                                    <i class="fas fa-sort-up ml-1 text-purple-600"></i>
                                @else
                                    <i class="fas fa-sort-down ml-1 text-purple-600"></i>
                                @endif
                            @endif
                        </a>
                    </th>
                ...
                @forelse($tesKemampuan as $tes)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $tes->pelamar->nama }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $tes->pelamar->periode->nama_periode }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <i class="far fa-calendar-alt text-indigo-500 mr-1"></i>
                                <span class="text-sm text-gray-900">{{ $tes->jadwal->format('d M Y') }}</span>
                            </div>
                            <div class="flex items-center">
                                <i class="far fa-clock text-indigo-500 mr-1"></i>
                                <span class="text-sm text-gray-500">{{ $tes->jadwal->format('H:i') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="h-4 w-full bg-gray-200 rounded-full overflow-hidden">
                                @php
                                    $scorePercentage = $tes->skor;
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full
                                ...
                                <i class="fas
                                    @if($tes->status_seleksi === 'Pending') fa-clock @endif
                                    @if($tes->status_seleksi === 'Tidak Lulus') fa-times-circle @endif
                                    @if($tes->status_seleksi === 'Lulus') fa-check-circle @endif
                                    @if($tes->status_seleksi === 'Magang') fa-graduation-cap @endif
                                    mr-1"></i>
                                {{ $tes->status_seleksi }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex space-x-2 justify-end">
                                <a href="{{ route('tes-kemampuan.show', $tes) }}" class="text-blue-600 hover:text-blue-900 transition-colors duration-200" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('tes-kemampuan.edit', $tes) }}" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-200" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('tes-kemampuan.destroy', $tes) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this test?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="mt-4">
        @if(method_exists($tesKemampuan, 'links'))
            {{ $tesKemampuan->appends(request()->except('page'))->links() }}
        @endif
    </div>
